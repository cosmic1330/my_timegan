...
def timegan(ori_data, parameters):
    ...
    X_tilde = recovery(H, T)
     # 表明期望的输入是None個 24X6维向量。
    print("X:", X.get_shape(),"H:", H.get_shape(),"X_tilde:", X_tilde.get_shape()) 

    # Generator
    E_hat = generator(Z, T)
    H_hat = supervisor(E_hat, T)
    H_hat_supervise = supervisor(H, T)
    print("E_hat:", E_hat.get_shape(),"H_hat:", H_hat.get_shape(),"H_hat_supervise:", H_hat_supervise.get_shape())
    ...
    print("Y_fake:", Y_fake.get_shape(), "Y_real:", Y_real.get_shape(),"Y_fake_e:", Y_fake_e.get_shape(), "\n")
    ...
    # 3. Two Momments
    G_loss_V1 = tf.reduce_mean(tf.abs(tf.sqrt(tf.nn.moments(X_hat, [0])[
                               1] + 1e-6) - tf.sqrt(tf.nn.moments(X, [0])[1] + 1e-6)))
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for itt in range(iterations):
            # Set mini-batch
            # 從原始資料中隨機抽取128(batch_size)筆資料做為每次batch的資料
            X_mb, T_mb = batch_generator(ori_data, ori_time, batch_size) 

            # Train embedder
            _, step_e_loss = sess.run(
                [E0_solver, E_loss_T0], feed_dict={X: X_mb, T: T_mb})
        ...
        generated_data_curr = sess.run(
            X_hat, feed_dict={Z: Z_mb, X: ori_data, T: ori_time})
